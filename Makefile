...
test_checker: clean_test
	cd src/ &&\
	python run.py test CheckerSuite 2>&1 | grep -E "(FAIL|OK|Ran|^AssertionError: checker)"
...
prepare_submission: build_antlr
	...
	cp -f src/main/d96/checker/StaticCheck.py submission/
	cp -f src/test/CheckerSuite.py submission/
